...
<template>
  <div>
    <table-search @submit="searchAssetsAccountPositionFlow">
      <slot>
    ...
    <easy-table :columns="tableColumns" :data="tableData" @current-change="(currentRow) => {selectedRow = currentRow}">
      <slot>
        <el-button-group>
          <el-button size="small" type="primary" @click="searchAssetsAccountPositionFlow" icon="el-icon-refresh">刷新</el-button>
        </el-button-group>
      </slot>
    </easy-table>
  </div>
...
<script>
  ...
  export default {
    ...
    mounted () {
      this.searchAssetsAccountPositionFlow()
    },
    methods: {
      searchAssetsAccountPositionFlow() {
        this.$post('prodL.2.81.json', {
