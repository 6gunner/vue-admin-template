...
<template>
  <div>
    <table-search @submit="searchCompanyOrder">
      <slot>
        <el-form-item label="">
          <el-date-picker
    ...
    <easy-table :columns="tableColumns" :data="tableData" @current-change="(currentRow) => {selectedRow = currentRow}">
      <slot>
        <el-button-group class="btn-left">
          <el-button size="small" type="primary" @click="searchCompanyOrder" icon="el-icon-refresh">刷新
          </el-button>
        </el-button-group>
        <el-button-group class="btn-right">
          <el-button size="small" type="primary" @click="exportCompanyOrder" icon="el-icon-export">数据导出
          </el-button>
        </el-button-group>
      </slot>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      export2Excel() {
        const tHeader = ['订单序号', '订单时间', '机构名称', '交易市场', '币对名称', '订单方向', '订单价格', '订单数量', '成交额', '成交均价', '订单状态'];
        const filterVal = ['row_id', 'create_date', 'co_name',  'exch_name', 'stock_name', 'order_dir', 'order_price', 'order_qty', 'bs_crncy_strike_qty', 'strike_aver_price', 'order_status'];
        ...
        const excelName = '机构委托' + currentDate.format('yyyyMMdd');
        const excelSheetName = '机构委托';
