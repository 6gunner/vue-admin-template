...
<template>
  <div>
    <table-search @submit="searchExchGroupOrder">
      <slot>
        <el-form-item label="">
          <el-date-picker
    ...
    <easy-table :columns="tableColumns" :data="tableData" @current-change="(currentRow) => {selectedRow = currentRow}">
      <slot>
        <el-button-group class="btn-left">
          <el-button size="small" type="primary" @click="exportExchGroupOrder" icon="el-icon-refresh">刷新
          </el-button>
        </el-button-group>
      </slot>
      <slot>
        <el-button-group class="btn-right">
          <el-button size="small" type="primary" @click="exportExchGroupOrder" icon="el-icon-export">数据导出
          </el-button>
        </el-button-group>
      </slot>
    </easy-table>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.searchExchGroupOrder()
    },
    methods: {
      searchExchGroupOrder() {
        this.$post('tdsecuL.4.113.json', {
          begin_date: this.formData.begin_end[0],
          end_date: this.formData.begin_end[1],
          pd_no: this.formData.pd_no,
          exch_group_no:this.exch_group_no,
          row_count: -1,
          sort_type:2
        }).then(resp => {
          this.tableData = resp
      ...
      exportExchGroupOrder() {
        this.export2Excel()
      },
      export2Excel() {
        const tHeader = ['订单序号', '订单时间', '机构名称', '机构账户名称', '业务账户名称', '交易账户名称','交易市场', '币对名称', '订单方向', '订单价格', '订单数量', '成交额', '成交均价', '订单状态'];
        const filterVal = ['row_id', 'create_date', 'co_name', 'pd_name', 'asset_acco_name', 'exch_group_name','exch_name', 'stock_name', 'order_dir', 'order_price', 'order_qty', 'bs_crncy_strike_qty', 'strike_aver_price', 'order_status'];
        ...
        const excelName = '交易账户委托' + currentDate.format('yyyyMMdd');
        const excelSheetName = '交易账户委托';
