...
<template>
  <div>
    <table-search @submit="searchExchGroupStrike">
      <slot>
        <el-form-item label="">
          <el-date-picker
    ...
    <easy-table :columns="tableColumns" :data="tableData" @current-change="(currentRow) => {selectedRow = currentRow}">
      <slot>
        <el-button-group class="btn-left">
          <el-button size="small" type="primary" @click="searchExchGroupStrike" icon="el-icon-refresh">刷新
          </el-button>
        </el-button-group>
      </slot>
      <slot>
        <el-button-group class="btn-right">
          <el-button size="small" type="primary" @click="exportExchGroupStrike" icon="el-icon-export">数据导出
          </el-button>
        </el-button-group>
      </slot>
    </easy-table>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.searchExchGroupStrike()
    },
    methods: {
      searchExchGroupStrike() {
        this.$post('tdsecuL.5.133.json', {
      ...
      },exportExchGroupStrike() {
        this.export2Excel()
      },
      export2Excel() {
        const tHeader = ['成交序号', '成交时间', '机构名称', '机构账户名称', '业务账户名称','交易账户名称', '交易市场', '币对名称', '订单方向', '成交价格', '成交数量', '成交额', '成交费用'];
        const filterVal = ['row_id', 'create_date', 'co_name', 'pd_name', 'asset_acco_name', 'exch_group_name', 'exch_name', 'stock_name', 'order_dir', 'strike_price', 'bs_crncy_strike_qty', 'qt_crncy_strike_qty', 'strike_fee'];
        ...
        const excelName = '交易账户成交' + currentDate.format('yyyyMMdd');
        const excelSheetName = '交易账户成交';
